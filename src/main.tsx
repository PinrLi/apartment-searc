  ...
  return (
    <>
      ...
      <nav aria-label="Main navigation">
        {["dashboard", "buildings", "units", "add", "rejected", "settings"].map(
          (p) => (
            <a key={p} className={page === p ? "selected" : ""} href={"#" + p}>
              {p === "add" ? "+ Add Apartment" : title(p)}
            </a>
          ),
        )}
      </nav>
...
function Detail({
  ...
  return (
    <>
      ...
      <h2>
        Units <span className="count">{b.units.length}</span>
      </h2>
      {b.units.map((u) => (
        <section key={u.id}>
          <div className="section-head">
            <h3>
              #{u.unit_number || "Unnamed"} ·{" "}
              {u.floorplan_name || "No floorplan name"}{" "}
              {!u.active && "(inactive)"}
            </h3>
            <div className="actions">
              <a className="button" href={`#add/${id}/${u.id}`}>
                Edit unit
              </a>
              <button className="danger subtle" onClick={() => remove(u)}>
                Delete unit
              </button>
            </div>
          </div>
          <p>
            {u.sqft ?? "?"} sqft · Available {u.available_date ?? "unknown"} ·{" "}
            {u.lease_length_months ?? "?"} months · Floor {u.floor ?? "?"} ·{" "}
            {u.orientation_or_view || "View unknown"}
          </p>
          <p>
            Base {money(u.base_rent)} + parking {money(u.parking_cost)} + fees{" "}
            {money(u.mandatory_monthly_fees)}
          </p>
          <UnitEvaluation u={u} />
          <p className="prewrap">{u.notes}</p>
          {u.concession_text && <p>Concession: {u.concession_text}</p>}
          <small>
            First seen {day(u.first_seen_date)} · Last seen{" "}
            {day(u.last_seen_date)} · Updated {day(u.last_updated)}
          </small>
        </section>
      ))}
      <section>
...
function Editor({
  ...
  function useExisting(update: boolean) {
    if (!duplicate) return;
    setBuilding(duplicate);
    setBuildingId(duplicate.id);
    setUpdateBuilding(update);
    setStep(2);
    setError("");
  }
